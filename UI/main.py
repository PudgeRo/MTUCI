...
class MainWindow(QWidget):
    ...
    def _create_shedule_tab(self):
        ...
        self.ins_btn = QPushButton("Вставить")
    ...
    def _update_table(self, table_gbox):
        self.cursor.execute(
            "SELECT * FROM timetable.timetable WHERE day='{}' and (week='{}' OR week='2')".format(table_gbox, week()))
        ...
        for i, r in enumerate(records):
            r = list(r)
            joinButton = QPushButton("Добавить " + '{} {}'.format(i, self.day_shorts[table_gbox]))
            delButton = QPushButton("Удалить " + '{} {}'.format(i, self.day_shorts[table_gbox]))
    ...
    def _change_day_from_table(self, button):
        ...
        try:
            ...
            self.cursor.execute(
                "UPDATE timetable.timetable SET subject='{0}', room_numb='{1}', start_time='{2}', week='{3}' "
    ...
    def _del_row_table(self, button):
        ...
        try:
            ...
            QMessageBox.about(self, "Ошибка", "Ошибка удаления")

    def _insert_row_table(self):
        try:
            print("Введите данные в формате: id, day, subject, room_numb, start_time, week")
            ...
            QMessageBox.about(self, "Ошибка", "Ошибка вставки")
